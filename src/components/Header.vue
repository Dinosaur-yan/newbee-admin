<template>
  <div class="header">
    <div class="left">
      <el-icon v-if="state.hasBack" @click="back"><Back /></el-icon>&nbsp;
      <span style="font-size: 20px">{{ state.name }}</span>
    </div>
    <div class="right">
      <el-popover placement="bottom" :width="320" trigger="click" popper-class="popper-user-box">
        <template #reference>
          <div class="author">
            <i class="icon el-icon-s-custom"></i>
            {{ (state.userInfo && state.userInfo.nickName) || '' }}
            <i class="el-icon-caret-bottom"></i>
          </div>
        </template>
        <div class="nickname">
          <p>登录名：{{ (state.userInfo && state.userInfo.loginUserName) || '' }}</p>
          <p>昵称：{{ (state.userInfo && state.userInfo.nickName) || '' }}</p>
          <el-tag effect="dark" class="logout" @click="logout">退出</el-tag>
        </div>
      </el-popover>
    </div>
  </div>
</template>

<script setup lang="ts">
import { onMounted, reactive } from 'vue'
import { useRouter } from 'vue-router'
import { Back } from '@element-plus/icons-vue'
import { localGet, localRemove } from '@/utils'
import axios from '@/utils/axios'

const router = useRouter()

const state = reactive({
  name: '首页',
  userInfo: {
    loginUserName: '',
    nickName: ''
  },
  hasBack: false
})

router.afterEach((to) => {
  const { name } = to
  if (name) {
    state.name = name.toString()
    state.hasBack = ['level2', 'level3'].some((val) => to.path.endsWith(val))
  }
})

onMounted(() => {
  const userInfo = localGet('userInfo')
  if (userInfo) {
    state.userInfo = userInfo
  }
})

const back = () => {
  router.back()
}

const logout = () => {
  axios.delete('/logout').then(() => {
    localRemove('token')
    router.push({ path: '/login' })
  })
}
</script>

<style scoped>
.header {
  height: 50px;
  border-bottom: 1px solid #e9e9e9;
  display: flex;
  justify-content: space-between;
  align-items: center;
  padding: 0 20px;
}
.right > div > .icon {
  font-size: 18px;
  margin-right: 6px;
}
.author {
  margin-left: 10px;
  cursor: pointer;
}
</style>

<style>
.popper-user-box {
  background: url('https://s.yezgea02.com/lingling-h5/static/account-banner-bg.png') 50% 50%
    no-repeat !important;
  background-size: cover !important;
  border-radius: 0 !important;
}
.popper-user-box .nickname {
  position: relative;
  color: #ffffff;
}
.popper-user-box .nickname .logout {
  position: absolute;
  right: 0;
  top: 0;
  cursor: pointer;
}
</style>
